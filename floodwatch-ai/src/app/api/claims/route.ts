 ...
 * /api/claims
 * 
 * GET: Returns recent evidence summary.
 * POST: AGENTIC citizen claims assistant (Gemini 1.5-flash).
 */
...
export async function GET() {
  ...
  return NextResponse.json(service.evidence);
}
...
export async function POST(req: Request) {
  try {
    ...
    const chatHistory = [
      ...
      { role: 'model' as const, parts: [{ text: 'Understood. I am the FloodWatch agentic assistant. I will help citizens report issues AND automatically verify their claims against live sensor data.' }] },
    ];
    ...
    const hasLocation = messages?.length >= 2;
    ...
    try {
      ...
      if (summary.length > 0) {
        const histLines = summary.map(s =>
          `${s.sensor_id} ${s.metric}: min=${s.min_value?.toFixed(2)}, max=${s.max_value?.toFixed(2)}, avg=${s.avg_value?.toFixed(2)} ${s.unit || ''}`
        ).join('\n');
        historicalSection = `\n\nHISTORICAL_EVIDENCE (past 72h):\n${histLines}`;
        if (patterns.length > 0) {
          const patternLines = patterns.map(p => `[${p.severity}] ${p.pattern_type}: ${p.description}`).join('\n');
          historicalSection += `\n\nDETECTED_PATTERNS:\n${patternLines}`;
        }
      }
